...
breed [ wolves wolf ]
breed [ red-wolves red-wolf ]
breed [ werewolves werewolf ]
breed [ vampires vampire ]
...
to setup
  ...
  ifelse model-version = "sheep-wolves-grass" [
    ask patches [
      set pcolor one-of [ green brown red ]
      ifelse pcolor = green
        [ set countdown grass-regrowth-time ]
      [ set countdown random grass-regrowth-time ] ; initialize grass regrowth clocks randomly for brown patches
      ifelse pcolor = red
       [ set countdown deadly-weed-regrowth-time ]
      [ set countdown random deadly-weed-regrowth-time ] ; initialize grass regrowth clocks randomly for brown patches
    ]
  ]
  [
  ...
  create-red-wolves initial-number-red-wolves  ; create the wolves, then initialize their variables
  [
    set shape "wolf"
    set color red
  ...
  create-werewolves initial-number-werewolves  ; create the wolves, then initialize their variables
  [
    set shape "wolf"
    set color blue
    set size 2  ; easier to see
  ...
  create-vampires initial-number-vampires  ; create the wolves, then initialize their variables
  [
    set shape "wolf"
    set color cyan
    set size 2  ; easier to see
...
to go
  ...
  ask red-wolves [
    ...
    eat-sheep ; wolves eat a sheep on their patch
    death ; wolves die if they run out of energy
    reproduce-red-wolves ; wolves reproduce at a random rate governed by a slider
  ]

  ask werewolves [
    move
    set energy energy - 1  ; wolves lose energy as they move
    eat-wolves ; wolves eat a sheep on their patch*
    eat-red-wolves
    death ; wolves die if they run out of energy
    reproduce-werewolves ; wolves reproduce at a random rate governed by a slider
  ]
  ...
  ask vampires [
    ...
    eat-red-wolves
    death ; wolves die if they run out of energy
    reproduce-vampires ; wolves reproduce at a random rate governed by a slider
  ]

  if model-version = "sheep-wolves-grass" [ ask patches
    [ ifelse random 100 < 70
    [ grow-grass ]
    [ grow-deadly-weed ]
    ]
  ]
  ;if model-version = "sheep-wolves-grass" [ ask patches [ grow-deadly-weed ] ]
...
to eat-deadly-weed  ; sheep procedure
  ; sheep eat grass and turn the patch brown
  if pcolor = red [
    set pcolor brown
...
to reproduce-red-wolves  ; wolf procedure
  if random-float 100 < red-wolf-reproduce [  ; throw "dice" to see if you will reproduce
    set energy (energy / 2)               ; divide energy between parent and offspring
    hatch 1 [ rt random-float 360 fd 1 ]  ; hatch an offspring and move it forward 1 step
  ]
end

to reproduce-werewolves  ; wolf procedure
  if random-float 100 < werewolf-reproduce [  ; throw "dice" to see if you will reproduce
    set energy (energy / 2)               ; divide energy between parent and offspring
    hatch 1 [ rt random-float 360 fd 1 ]  ; hatch an offspring and move it forward 1 step
  ]
end

to reproduce-vampires  ; wolf procedure
  if random-float 100 < vampire-reproduce [  ; throw "dice" to see if you will reproduce
    set energy (energy / 2)               ; divide energy between parent and offspring
    hatch 1 [ rt random-float 360 fd 1 ]  ; hatch an offspring and move it forward 1 step
  ]
end
...
to eat-red-wolves  ; wolf procedure
  let prey one-of red-wolves-here                    ; grab a random sheep
  if prey != nobody  [                          ; did we get one? if so,
    ask prey [ die ]                            ; kill it, and...
    set energy energy + wolf-gain-from-food     ; get energy from eating
  ]
...
to grow-deadly-weed  ; patch procedure
...
to display-labels
  ...
  if show-energy? [
    ...
    ask red-wolves [ set label round energy ]
    ask werewolves [ set label round energy ]
    ask vampires [ set label round energy ]
...
"red wolves" 1.0 0 -2674135 true "" "plot count red-wolves"
"werewolves" 1.0 0 -13345367 true "" "plot count werewolves"
"vampires" 1.0 0 -11221820 true "" "plot count vampires"
"grass / 4" 1.0 0 -14333415 true "" "if model-version = \"sheep-wolves-grass\" [ plot count grass / 4 ]"
"deadly-weed" 1.0 0 -10146808 true "" "if model-version = \"sheep-wolves-grass\" [ plot count deadly-weed / 4 ]"
